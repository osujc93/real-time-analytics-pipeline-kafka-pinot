...
class USZipcodeLocationData:
    ...
    STATE_AREA_CODES = {
        "AL": [205, 251, 256, 334, 938, 659],
        "AK": [907],
        "AZ": [480, 520, 602, 623, 928],
        "AR": [327, 479, 501, 870],
        "CA": [209, 213, 279, 310, 323, 341, 350, 369, 408, 415, 424, 442, 510, 530, 559, 562, 619, 626, 628, 650, 657, 661, 669, 707, 714, 738, 747, 752, 760, 805, 818, 820, 831, 837, 840, 858, 909, 916, 925, 949, 951],
        "CO": [303, 719, 720, 970, 983],
        "CT": [203, 475, 860, 959],
        "DE": [302],
        "FL": [239, 305, 321, 324, 352, 386, 407, 448, 561, 627, 645, 652, 656, 689, 727, 728, 754, 772, 786, 813, 850, 863, 904, 941, 954],
    ...
    def init_uszipcode_data(cls):
        ...
        try:
            cls._all_zips = search.by_population(
                lower=0, upper=2_147_483_647, returns=50_000
            )
        except TypeError:
            cls._all_zips = search.by_population(
                lower=0, upper=2_147_483_647, limit=50_000
            )
    ...
    def get_random_location(cls):
        if not cls._initialized or not cls._all_zips:
            raise RuntimeError(
                "Location data not initialized. Call init_uszipcode_data() first."
            )

        z = random.choice(cls._all_zips)
        city = getattr(z, "major_city", z.city)
        state = z.state
        zipcode = z.zipcode

        area_code = random.choice(cls.STATE_AREA_CODES.get(state, [999]))

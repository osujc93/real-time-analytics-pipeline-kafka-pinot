...
sudo chown -R zookeeper2:zookeeper /data /logs /datalog /conf/ssl
...
if [ ! -d "$KEYTAB_EXT4_DIR" ]; then
    ...
    sudo chown -R zookeeper2:zookeeper "$KEYTAB_EXT4_DIR"
fi

# Move keytabs to the ext4 filesystem directory and ensure proper permissions
if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-server2.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-server2.keytab ${KEYTAB_EXT4_DIR}/zookeeper-server2.keytab
fi
sudo chown zookeeper2:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-server2.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-server2.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum2.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum2.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum2.keytab
fi
sudo chown zookeeper2:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum2.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum2.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client2.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum-client2.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client2.keytab
fi
sudo chown zookeeper2:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client2.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client2.keytab

# Compile BouncyCastle Jar

sudo /apache-zookeeper-3.8.4-bin/bin/setup_bouncycastle_agent.sh

sudo chown zookeeper2:zookeeper /apache-zookeeper-3.8.4-bin/lib/BouncyCastleAgent-fat.jar

...
sudo chown -R zookeeper3:zookeeper /data /logs /datalog /conf/ssl
...
if [ ! -d "$KEYTAB_EXT4_DIR" ]; then
    ...
    sudo chown -R zookeeper3:zookeeper "$KEYTAB_EXT4_DIR"
fi

# Move keytabs to the ext4 filesystem directory and ensure proper permissions
if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-server3.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-server3.keytab ${KEYTAB_EXT4_DIR}/zookeeper-server3.keytab
fi
sudo chown zookeeper3:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-server3.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-server3.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum3.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum3.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum3.keytab
fi
sudo chown zookeeper3:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum3.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum3.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client3.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum-client3.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client3.keytab
fi
sudo chown zookeeper3:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client3.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client3.keytab
...
sudo chown zookeeper3:zookeeper /apache-zookeeper-3.8.4-bin/lib/BouncyCastleAgent-fat.jar

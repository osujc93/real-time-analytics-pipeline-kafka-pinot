...
sudo chown -R zookeeper1:zookeeper /data /logs /datalog /conf/ssl
...
if [ ! -d "$KEYTAB_EXT4_DIR" ]; then
    ...
    sudo chown -R zookeeper1:zookeeper "$KEYTAB_EXT4_DIR"
fi

# Move keytabs to the ext4 filesystem directory and ensure proper permissions
if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-server1.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-server1.keytab ${KEYTAB_EXT4_DIR}/zookeeper-server1.keytab
fi
sudo chown zookeeper1:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-server1.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-server1.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum1.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum1.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum1.keytab
fi
sudo chown zookeeper1:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum1.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum1.keytab

if [ ! -f ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client1.keytab ]; then
    sudo cp /etc/krb5kdc/keytabs/zookeeper-quorum-client1.keytab ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client1.keytab
fi
sudo chown zookeeper1:zookeeper ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client1.keytab
sudo chmod 640 ${KEYTAB_EXT4_DIR}/zookeeper-quorum-client1.keytab

# Compile BouncyCastle Jar

sudo /apache-zookeeper-3.8.4-bin/bin/setup_bouncycastle_agent.sh

sudo chown zookeeper1:zookeeper /apache-zookeeper-3.8.4-bin/lib/BouncyCastleAgent-fat.jar

...
check_secure_client_port() {
    log_message "Checking if secure client port 2281 is open locally..."

    # Get the IP address for this node
    local_ip=$(hostname -I | awk '{print $1}')

    # Use ss to check if port 2281 is listening on the specific IP address
    if ss -ltn | grep "$local_ip:2281" > /dev/null 2>&1; then
        log_message "Port 2281 is listening on IP $local_ip."
    else
        log_message "Port 2281 is not listening on IP $local_ip!"
        return 1
    fi
}
...
check_mntr_command() {
    ...
    output=$(echo "mntr" | openssl s_client -connect "$(hostname):2281" \

...
if [ "$ZK_IS_CLIENT" = "true" ]; then
    ...
    KEYTAB="/root/.ssh/zookeeper_keytabs/zookeeper-quorum-client2.keytab"
    PRINCIPAL="zookeeper-quorum-client2@EXAMPLE.COM"
    ...
    KEYTAB_SERVER="/root/.ssh/zookeeper_keytabs/zookeeper-server2.keytab"
    PRINCIPAL_SERVER="zookeeper-server/zookeeper-server2@EXAMPLE.COM"
    
    KEYTAB_QUORUM="/root/.ssh/zookeeper_keytabs/zookeeper-quorum2.keytab"
    PRINCIPAL_QUORUM="zookeeper-quorum-server/zookeeper-quorum-server2@EXAMPLE.COM"
    
    PRINCIPAL_LEARNER="zookeeper-quorum-learner/zookeeper-quorum-learner2@EXAMPLE.COM"
...
-Dzookeeper.quorum.auth.kerberos.servicePrincipal=zookeeper-quorum-server/zookeeper-quorum-server2@EXAMPLE.COM \
...
-Dzookeeper.ssl.keyStore.location=/opt/bitnami/zookeeper/conf/ssl/zookeeper-quorum-server2.keystore.p12 \
-Dzookeeper.ssl.keyStore.password=NeloNELO123456789 \
-Dzookeeper.ssl.keyStore.type=PKCS12 \
-Dzookeeper.ssl.trustStore.location=/opt/bitnami/zookeeper/conf/ssl/zookeeper-quorum-server2.truststore.p12 \
-Dzookeeper.ssl.trustStore.password=NeloNELO123456789 \
-Dzookeeper.ssl.trustStore.type=PKCS12 \
-Dzookeeper.ssl.quorum.keyStore.location=/opt/bitnami/zookeeper/conf/ssl/zookeeper-quorum-server2.keystore.p12 \
-Dzookeeper.ssl.quorum.keyStore.password=NeloNELO123456789 \
-Dzookeeper.ssl.quorum.trustStore.location=/opt/bitnami/zookeeper/conf/ssl/zookeeper-quorum-server2.truststore.p12 \
...
-Dzookeeper.server.principal=zookeeper-server/zookeeper-server2@EXAMPLE.COM \

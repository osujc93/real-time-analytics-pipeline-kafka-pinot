...
sudo ln -s /conf/zookeeper_keytabs/zookeeper-quorum1.keytab /etc/krb5.keytab

# Path to zoo.cfg on the local node
ZOO_CFG_PATH="/conf/zoo.cfg"
...
# Logging (optional)
LOG_FILE="/logs/zookeeper_rolling_upgrade.log"
...
restart_zookeeper() {
    ...
    /apache-zookeeper-3.9.3-bin/bin/zkServer.sh stop
    ...
    sleep 5
    /apache-zookeeper-3.9.3-bin/bin/zkServer.sh start
    sleep 45
}
...
verify_zookeeper() {
    ...
    while (( attempt < max_attempts )); do
        response=$(/apache-zookeeper-3.9.3-bin/bin/zkServer.sh status 2>/dev/null)
...
update_node() {
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') - Updating configuration files on local node..."

    # Check if the configuration files exist
    if [ ! -f "/conf/zoo-sasl-${NODE_NUM}.cfg" ] || [ ! -f "/usr/lib/jvm/java-17-openjdk-amd64/conf/security/java${NODE_NUM}.security" ]; then
        echo "Configuration files for local node are missing."
        exit 1
    fi

    # Replace the configuration files with node-specific versions
    cp "/conf/zoo-sasl-${NODE_NUM}.cfg" "$ZOO_CFG_PATH"
    ...
    cp "/apache-zookeeper-3.9.3-bin/bin/zkEnv-sasl-${NODE_NUM}.sh" "/apache-zookeeper-3.9.3-bin/bin/zkEnv.sh"

    # Initialize Kerberos credential cache
    kinit -k -t /conf/zookeeper_keytabs/zookeeper-quorum1.keytab zookeeper-quorum-server/zookeeper-quorum-server1@EXAMPLE.COM

    # Verify the ticket
    klist -c /conf/krb5cc/zookeeper-cc_1

    sudo chown zookeeper1:zookeeper /conf/krb5cc/zookeeper-cc_1

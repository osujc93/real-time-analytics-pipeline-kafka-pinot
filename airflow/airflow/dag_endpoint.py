...
def get_dag_details(
    *, dag_id: str, fields: Collection[str] | None = None, session: Session = NEW_SESSION
    ...
    if hasattr(dag_model, "params") and isinstance(dag_model.params, list):
        warnings.warn(
            "Detected dag_model.params is a list instead of a dict. "
            "Converting it to a dummy dict so Marshmallow won't fail."
        )
        new_params = {}
        for i, val in enumerate(dag_model.params):
            new_params[f"param_{i}"] = val
        dag_model.params = new_params
    ...
    try:
        ...
        raise BadRequest(
            "DAGDetailSchema serialization error",
            detail=(
                f"Encountered a Marshmallow serialization issue. Original error: {e}. "
                "Check DAG params/tags for non-dict data."
            ),
        )
